...
const pendingData = new Map(); // sender -> { type, details: string | {}, orderId: null, name: '', filling: '', deliveryDate: '' }
...
async function routeExistingUser(sender, text, isImage) {
  ...
  if (state === "AWAITING_FILLING") {
    ...
    if (choice >= 1 && choice <= FILLINGS.length) {
      ...
      await sock.sendMessage(sender, { text: "الرجاء إدخال تاريخ استلام الطلبية (مثال: 2025-09-30) 📅" });
      respondedMessages.set(sender, "AWAITING_DELIVERY_DATE");
      return;
    } else if (text === "0") {
      respondedMessages.set(sender, "MAIN_MENU");
      pendingData.delete(sender);
  ...
  if (state === "AWAITING_DELIVERY_DATE") {
    pendingData.set(sender, { ...pendingData.get(sender), deliveryDate: text });
...
✨ للتنويه ✨  
⏰ يكون وقت تسليم الطلبية قبل ٢٤ ساعة من وقت الطلب  
☕ خلال أوقات عمل المقهى:  
   🕑 من ٢:٠٠ م إلى ١٢:٠٠ ص  
📅 يوم الجمعة:  
   🕓 من ٤:٠٠ م إلى ١٢:٠٠ ص`;
  await sock.sendMessage(jid, { 
    text,
    linkPreview: {
...
async function submitOrderForReview(jid) {
  ...
  if (data.deliveryDate) {
    details += `\nتاريخ الاستلام: ${data.deliveryDate}`;
  }
